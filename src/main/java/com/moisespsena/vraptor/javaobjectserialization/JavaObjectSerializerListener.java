...
@FullLazy
@RequestSerializer
public class JavaObjectSerializerListener implements RequestSerializerListener {

	@Override
	public boolean accepts(final RequestSerializerListenerExecutor executor) {
		final String contentType = executor.getRequestInfo().getRequest()
				.getContentType();
		if ((contentType != null)
				&& contentType.contains("application/x-java-serialized-object")) {
			return true;
		} else {
			return false;
		}
	...
	public Class<? extends Serialization> getSerializationClass(
		...
		return JavaObjectSerialization.class;
	}
}
